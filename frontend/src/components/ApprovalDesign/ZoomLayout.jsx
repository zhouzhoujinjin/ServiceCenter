...
const ZoomLayout = (props) => {
  ...
  useEffect(() => {
    if (snapping) {
      const id = setTimeout(() => {
        toPng(document.getElementById('box-scale'), { backgroundColor: '#ffffff' }).then(function (dataUrl) {
  ...
  return (
    <>
      <div className={`zoom${fullScreen ? ' fullscreen' : ''}`}>
        <Input.Group compact>
          <Button
            disabled={snapping}
            loading={snapping}
          ...
          <Button
            ...
            onClick={() => {
              if (fullScreen) {
                ...
                document.getElementsByClassName('flow-wrapper')[0].requestFullscreen()
                setFullScreen(true)
              }
            }}></Button>
          <Button
            ...
            icon={<FontAwesomeIcon icon={['fal', 'minus']} />}></Button>
          <Input onChange={handleChange} value={inputValue} suffix='%' style={{ width: 64 }}></Input>
          <Button
            disabled={scale === ZOOM.MAX}
            onClick={() => zoomSize(ZOOM.UP)}
            icon={<FontAwesomeIcon icon={['fal', 'plus']} />}></Button>
        </Input.Group>
      </div>
      <div
        className='box-scale'
