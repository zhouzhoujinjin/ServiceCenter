...
const ApprovalsWaitPage = () => {
  ...
  const [filters, setFilters] = useState({ actionType: 'pending' })
        ...
        <>
          <Radio.Group
            onChange={(e) => {
              ...
              setFilters({ actionType: 'pending', templateName: k })
              setCurrentPage(1)
            }}
            ...
            {options.map((o) => (
              <Badge
                key={o.value}
                count={(counts && counts.hasOwnProperty(o.value) && counts[o.value]) || 0}
                offset={[-4, 2]}
                size='small'>
...
export default ApprovalsWaitPage
