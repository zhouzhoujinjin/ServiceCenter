...
namespace Approval.Managers
{
  public class TemplateManager
  {
    ...
    public async Task<IEnumerable<ApprovalTemplateEntity>> GetUserTemplates(User user)
    {
        ...
        else
        {
          ...
          if (deptCount != 0)
          {
            if (user.Profiles.ContainsKey("departmentIds"))
            {
              ...
              {
                var allowDeptApply = template.Applicants.Departments.Any(x => x.Id == int.Parse(id));
                if (allowDeptApply)
                {
                  isExist = true;
                  break;
    ...
    /// <returns></returns>
    public async Task<IEnumerable<ApprovalTemplateEntity>> GetUserApprovalTemplates(User user)
    {
    ...
    /// <param name="user"></param>
    /// <returns></returns>
    public async Task<IEnumerable<ApprovalTemplateEntity>> GetUserCcTemplates(User user)
    {
      var approvals = await context.ApprovalNodes.Include(x => x.Item).ThenInclude(x => x.Template).Where(x => x.UserId == user.Id
    ...
    public async Task<IEnumerable<long>> GetDepartmentIdsAsync(long userId, string templateName)
    {
