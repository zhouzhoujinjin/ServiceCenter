...
namespace Approval.Managers
{
  public class ApprovalManager
  {
    ...
    /// <returns></returns>
    public async Task<IEnumerable<UserModel>> StartApproval(int itemId)
    {
    ...
    /// <param name="item"></param>
    /// <returns></returns>
    public async Task<List<UserModel>> GetPendingUsers(ApprovalItemEntity item)
    {
      ...
      {
        var user = await userManager.GetBriefUserAsync((ulong)userId);
        if (user != null)
        {
          users.Add(user);
        }
    ...
    /// <param name="itemId"></param>
    /// <returns></returns>
    public async Task<List<UserModel>> GetPendingUsers(int itemId)
    {
    ...
    /// <param name="itemId"></param>
    /// <returns></returns>
    public async Task<UserModel> GetCreator(int itemId)
    {
      var item = await context.ApprovalItems.FirstOrDefaultAsync(x => x.Id == itemId);
      if (item == null) return null;
      return await userManager.GetBriefUserAsync((ulong)item.CreatorId);
    }
    ...
    /// <param name="itemId"></param>
    /// <returns></returns>
    public async Task<IEnumerable<UserModel>> GetItemCcUsers(int itemId)
    {
      IEnumerable<UserModel> ts = new List<UserModel>();
    ...
    public async Task<List<UserModel>> GetApprovalUsers(ulong creatorId, Dictionary<string, string> filters)
    {
    ...
    private IQueryable<ApprovalItemEntity> GetApprovalUsersQuery(ulong currentUserId, Dictionary<string, string> filters)
    {
    ...
    /// <param name="itemId"></param>
    /// <returns></returns>
    public async Task<IEnumerable<ApprovalNodeEntity>> CreateApprovalNodes(IEnumerable<IApprovalFlowNode> flowNodes, ulong startUserId, int itemId, int oldId)
    {
    ...
    /// <returns></returns>
    public async Task<ApprovalItemEntity> CreateApprovalItem(string code, string title, long templateId, string templateName, ulong creatorId, Dictionary<string, string> content, ApprovalItemStatus status, bool isUpdate)
    {
    ...
    /// <param name="itemId"></param>
    /// <returns></returns>
    public async Task<ApprovalItemModel> GetItemInfo(int itemId, ulong userId = 0)
    {
    ...
    /// <returns></returns>
    public async Task<(int, IEnumerable<ApprovalItemModel>)> GetUserHistoryItems(ulong userId, Dictionary<string, string> filters, int page = 1, int size = 20)
    {
      ...
      foreach (var item in result)
      {
        ...
        {
          var u = item.Nodes.First().UserId > 0 ? await userManager.GetBriefUserAsync((ulong)item.Nodes.First().UserId,null, new string[] { "FullName" }) : null;
    ...
    /// <returns></returns>
    public async Task<(int, IEnumerable<ApprovalItemModel>)> GetUserApprovals(ulong userId, Dictionary<string, string> filters, int page = 1, int size = 20)
    {
    ...
    /// <param name="userId"></param>
    /// <returns></returns>
    public async Task<Dictionary<string, int>> GetUserApprovalCounts(ulong userId)
    {
    ...
    /// <param name="userId"></param>
    /// <returns></returns>
    public async Task<int> GetUserApplyApprovalCounts(ulong userId)
    {
      return await context.ApprovalNodes
        .Include(x => x.Item)
        .ThenInclude(x => x.Template)
    ...
    public async Task<Dictionary<string, int>> GetUserCcCounts(ulong userId)
    {
      //提示30天以内的
    ...
    /// <returns></returns>
    public async Task<bool> CheckUserCanComment(ulong userId, ApprovalNodeEntity currentNode)
    {
    ...
    /// <param name="userId"></param>
    /// <returns></returns>
    private IQueryable<ApprovalItemEntity> GetItemQuery(Dictionary<string, string> filters, ulong userId = 0)
    {
      var query = context.ApprovalItems
        .Include(e => e.Template)
        .Include(e => e.Nodes)
    ...
    /// <param name="userId"></param>
    /// <returns></returns>
    private IQueryable<ApprovalNodeEntity> GetNodeQuery(Dictionary<string, string> filters, ulong userId)
    {
      ...
      {
        var r = ulong.TryParse(filters["notCreatorId"], out ulong creatorId);
      ...
      {
        var userIds = JsonSerializer.Deserialize<List<ulong>>(filters["userIds"]);
    ...
    /// <param name="userId"></param>
    /// <returns></returns>
    public async Task<UserModel> GetUserDepartment(ulong userId, bool isMajor = true)
    {
      return await cache.GetAsync($"{Consts.UserProfileWithDepartmentCacheKey}:{userId}_{(isMajor ? "OnlyMajor" : "All")}", async () =>
         {
           ...
           var departments = await departmentManager.GetUserDepartments((ulong)userId);
    ...
    /// <returns></returns>
    private bool IsCurrentNode(ulong currentUserId, ApprovalNodeEntity node)
    {
    ...
    public async Task<ApprovalNodeEntity> InsertNode(ApprovalNodeModel node, int itemId, List<ApprovalNodeModel> nodes, int currentIndex, int seq)
    {
      ...
      var userIds = node.Children.Count() == 0 ? new List<ulong> { node.UserId } : node.Children.Select(x => x.UserId).ToList();
      if (userIds.Count() == 0)
      {
        return null;
      }
    ...
    /// <param name="userId"></param>
    /// <returns></returns>
    public async Task<ApprovalNodeEntity> FindUserCurrentNode(int itemId, ulong userId)
    {
      var item = await context.ApprovalItems.Include(x => x.Nodes).FirstOrDefaultAsync(x => x.Id == itemId);
    ...
    /// <param name="itemId"></param>
    /// <returns></returns>
    public async Task<IEnumerable<ulong>> GetItemNodesAllUsers(int itemId)
    {
    ...
    /// <param name="userId"></param>
    /// <returns></returns>
    public async Task<ApprovalNodeEntity> InsertTransNode(int nodeId, ulong userId, string comment, UserModel user)
    {
    ...
    public async Task<bool> IsUpdateCommentAsync(int nodeId, UserModel user, string comment)
    {
    ...
    public async Task<(int, IEnumerable<ApprovalItemModel>)> GetVerifyItems(Dictionary<string, string> filters, int page = 1, int size = 20)
    {
      ...
      foreach (var item in items)
      {
        var User = await userManager.GetBriefUserAsync(item.CreatorId.Value, null,new string[] { "public" });
        item.Creator = User;
      }
    ...
    //获得已经发布的所有申请
    private async Task<List<ApprovalItemModel>> ListPublishItemsAsync(ulong userId)
    {
    ...
    //获得已经读过的集合
    public async Task<List<int>> ListReadItemsAsync(ulong userId)
    {
      return await context.ApprovalReadLogs.Where(x => x.UserId == userId).Select(x => x.ItemId).ToListAsync();
    }

    //获得已经发布的公告是否有新的
    public async Task<bool> IsNewPublishItemsAsync(ulong userId)
    {
    ...
    //已读操作
    public async Task<bool> ReadItemAsync(int itemId, ulong userId)
    {
    ...
    //获取已经发布的公告通知
    public async Task<IEnumerable<ApprovalItemModel>> ListPublishItemsAsync(ulong userId, string baseUrl)
    {
      var list = await ListPublishItemsAsync(userId);
      var readIds = await ListReadItemsAsync(userId);
    ...
    /// <param name="itemId"></param>
    /// <returns></returns>
    public async Task<ApprovalItemEntity> CreateCopyItem(int itemId, ulong userId, string code)
    {
      var entity = await context.ApprovalItems.FirstOrDefaultAsync(x => x.Id == itemId);
      var newContent = entity.Content;
      var userDepartments = await departmentManager.GetUserDepartments(userId);
    ...
    public async Task<string> UpdateResponseCodeAsync(int itemId, ulong userId, string responseCode, string sourceDesc)
    {
    ...
    //更新抄送已读
    public async Task<string> UpdateNodeCcAsync(int itemId, ulong userId)
    {
    ...
    //找到流程中最后的审批人
    public async Task<IEnumerable<ulong>> GetFinallyUsersAsync(int itemId)
    {
      var userIds = new List<ulong>();
