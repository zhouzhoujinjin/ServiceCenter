...
namespace Approval.Controllers
{
  ...
  public class ApprovalController : ControllerBase
  {
    ...
    public ApprovalController(
      ...
      IDistributedCache cache, IOptions<WorkDomainOptions> workDomainOptionsAccessor)
    {
    ...
    public async Task<AjaxResp> GetUsersSelect()
    {
      ...
      {
        var users = await userManager.FindUsersAsync(new Dictionary<string, object>(), 0, 999);
        var departmentUsers = new List<DepartmentUserModel>();
        foreach (var user in users)
        {
          var userProfiles = (await approvalManager.GetUserDepartment(user.Id)).Profiles;
          if (userProfiles.ContainsKey("departmentId"))
          {
            departmentUsers.Add(new DepartmentUserModel
            {
              Id = user.Id,
    ...
    public async Task<AjaxResp> GetTransUsersSelect(int itemId)
    {
      ...
      //
      var deptUsers = new List<DepartmentUserSelect>();
      var deptQuery = departmentUsers.OrderBy(x => ulong.Parse(x.Profiles["departmentId"].ToString())).Select(x => new DepartmentModel { Id = ulong.Parse(x.Profiles["departmentId"].ToString()), Title = x.Profiles["departmentName"].ToString() });
      ...
      foreach (var dept in depts)
      {
        var tree = new DepartmentUserSelect
        {
          Value = (ulong)(dept.Id + 1000),
          Title = dept.Title,
          Children = departmentUsers.Where(x => ulong.Parse(x.Profiles["departmentId"].ToString()) == dept.Id)
          .OrderBy(x => x.Id)
          .Select(x => new DepartmentUserSelect
          {
            Value = ulong.Parse(x.Id.ToString()),
    ...
    [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme, Policy = PermissionClaimNames.ApiPermission)]
    public async Task<AjaxResp<IEnumerable<DepartmentUserModel>>> GetAccountDepartments()
    {
      var userId = HttpContext.GetUserId();
      var departments = await departmentManager.GetUserDepartments(userId);
      var user = await userManager.GetBriefUserAsync(userId);
      var userDepts = departments.Select(x => new DepartmentUserModel
      {
        UserName = user.UserName,
        Id = user.Id,
      ...
      return new AjaxResp<IEnumerable<DepartmentUserModel>>
      {
        Data = userDepts
      };
    }
    ...
    [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme, Policy = PermissionClaimNames.ApiPermission)]
    public async Task<AjaxResp<IEnumerable<DepartmentUserModel>>> GetUsers()
    {
      var users = await userManager.FindUsersAsync(new Dictionary<string, object>(), 0, 999);
      var departmentUsers = new List<DepartmentUserModel>();
      foreach (var user in users)
    ...
    [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme, Policy = PermissionClaimNames.ApiPermission)]
    public async Task<AjaxResp<IEnumerable<UserModel>>> GetApprovalUsers(string query = null)
    {
      var userId = HttpContext.GetUserId();
      ...
      return new AjaxResp<IEnumerable<UserModel>>
      {
        Data = users
      };
    }
    ...
    private async Task PostProcessNodeAysnc(ICollection<IApprovalFlowNode> flowNodes)
    {
      await flowNodes.ForEachAsync(async x =>
      {
        ...
        {
          (x as ApprovalFlowNode).User = await userManager.GetBriefUserAsync(x.UserId, null, new string[] { "public" });
        }
      });
    }
    ...
    public async Task<AjaxResp<ApprovalNodeModel>> UpdateNodeInfoAsync(int itemId, int nodeId, [FromBody] ApprovalNodeModel node)
    {
      ...
      if (!string.IsNullOrEmpty(node.Comment))
      {
        if (canComment)
        {
          ...
          var mentionUserIds = mc.Select(x => ulong.Parse(x.Groups[1].Value)).ToList();
      ...
      // slobber: ignore
      //await weChatWorkApi.SendTextCardMessageAsync("approval", creator.UserName, "", "", $"您收到了一条审批结果消息", $"{approval.Title} --{creatorMsg}", url, "点击查看详情");
    ...
    public async Task<AjaxResp<ApprovalItemModel>> PublishVerifiedFileItem(int itemId, [FromBody] PublishInfo info)
    {
      ...
      {
        //特定发送
        users = await userManager.GetBriefUsersAsync(info.Purview.Select(x => ulong.Parse(x)));
      }
      else
      {
        //全员能看
        users = await userManager.FindUsersAsync(new Dictionary<string, object>(), 0, 999);
      }
      // slobber: ignore
      // await weChatWorkApi.SendTextCardMessageAsync("approval", string.Join("|", users.Select(x => x.UserName)), "", "", info.PublishTitle, msg, $"{workDomainOptions.BaseUrl}/pages/notices/index/index", "点击查看");
      return new AjaxResp<ApprovalItemModel>
      {
        Data = new ApprovalItemModel
        {
    ...
    /// 发送消息给当前待审批的用户
    /// </summary>
    private async Task SendTemplateCard(UserModel creator, IEnumerable<UserModel> sendUsers, int itemId, string title, string approvalDate, string url, string sourceDesc = "审批消息")
    {
      //构造审批消息
